...
// contract VaribleYieldToken is IVaribleYieldToken, ERC20, ReentrancyGuard, RewardManager, InterestManagerYT {
    ...
    //         revert YieldContractInsufficientSy(totalSyToMint, _getFloatingSyAmount());
    ...
    //     if (!redeemInterest && !redeemRewards) revert YTNothingToRedeem();
    ...
    //     if (!isExpired()) revert YTNotExpired();
    ...
    //     if (postExpiry.firstPYIndex == 0) revert YTPostExpiryDataNotSet();
    ...
    //         IPrincipalToken(PT).mintByYT(receiverPTs[i], amountPYOuts[i]);
    ...
    //     address treasury = IYieldContractFactory(factory).treasury();
    ...
    //     if (amount == 0) revert YTNoFloatingSy();
    // }
    ...
    //     uint128 index128 = Math.max(IStandardizedYield(SY).exchangeRate(), _pyIndexStored).Uint128();
    ...
    //             ? IYieldContractFactory(factory).interestFeeRate()
    //             : Math.ONE;
    ...
    //         ? IYieldContractFactory(factory).rewardFeeRate()
    //         : Math.ONE;
